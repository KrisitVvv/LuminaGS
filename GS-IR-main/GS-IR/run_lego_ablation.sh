...
# 消融测试参数组合
NORMAL_TVS=(0.3)
NORMAL_LOSS_WEIGHTS=(0.12)
BRDF_TVS=(0.5)

for n_tv in "${NORMAL_TVS[@]}"; do
    for nl_w in "${NORMAL_LOSS_WEIGHTS[@]}"; do
        for bt_w in "${BRDF_TVS[@]}"; do
            # OUTPUT_PATH="outputs/lego_gt_n${n_tv}_nl${nl_w}_bt${bt_w}" 
            OUTPUT_PATH="outputs/lego_step4_freeze_adapted"
            echo "=========================================================================="
            ...
            # BRDF TV在此时一般不产生重大作用（材质属性未全面解锁），但保持传入可以避免参数悬空或未来兼容问题
            echo "--- Step 1: 基础训练 (30,000 Iterations) ---"
            python train.py -m $OUTPUT_PATH/ -s $SCENE_PATH --iterations 30000 --normal_tv ${n_tv} --normal_loss_weight ${nl_w} --brdf_tv ${bt_w} --eval
            ...
            # 这里是BRDF TV起决定性作用的阶段！必须将其命名为 "--brdf_tv" 而非 "--brdf_tv_weight"
            echo "--- Step 3: 物理反演微调 (35,000 Iterations) ---"
            python train.py -m $OUTPUT_PATH/ -s $SCENE_PATH --start_checkpoint $OUTPUT_PATH/chkpnt30000.pth --iterations 35000 --normal_tv ${n_tv} --normal_loss_weight ${nl_w} --brdf_tv ${bt_w} --eval --gamma --indirect
