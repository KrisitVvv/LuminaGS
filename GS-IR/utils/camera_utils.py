...
def loadCam(args: GroupParams, id: int, cam_info: CameraInfo, resolution_scale: float) -> Camera:
    ...
    return Camera(
        ...
        da3_depth=cam_info.da3_depth if not getattr(args, 'is_baseline', False) else None,
        da3_normal=cam_info.da3_normal if not getattr(args, 'is_baseline', False) else None,
    )
